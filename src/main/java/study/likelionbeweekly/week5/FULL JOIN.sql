...
DROP TABLE IF EXISTS Members;	-- 외래 키 제약조건 때문에 마지막에
...
INSERT INTO Members (UserID, password) VALUES ("yohan", "password123");
...
INSERT INTO Posts(PostID, title, content) VALUES("100", "RIGHT JOIN", "TEST");

INSERT INTO Comments VALUES("123", "minseok", "2", "후루후루", "1");
INSERT INTO Comments VALUES("234", "suji", "3", "김치 탕후루는 없나요?", "1");

	-- FULL JOIN -> Members와 Posts 테이블 조인 -> UNION사용 
SELECT Members.UserID, Members.email, Members.password, Posts.title, Posts.content, Posts.likes
FROM Members
LEFT JOIN Posts ON Members.UserID = Posts.UserID
UNION
SELECT Members.UserID, Members.email, Members.password, Posts.title, Posts.content, Posts.likes
FROM Members
RIGHT JOIN Posts ON Members.UserID = Posts.UserID;